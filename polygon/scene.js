class Scene {
  ...
  drawGrid() {
    ...
    for (let j = 0; j < this.height / 100; j++) {
      this.ctx.moveTo(0, j * 100)
      this.ctx.lineTo(this.width, j * 100)
      this.ctx.stroke()
    }
  }
}
...
class Polygon {
  ...
  initCalcPoints() {
    ...
    for (let i = 0; i < this.count; i++) {
      let x = Util.cos(this.angle + i * per) * this.radius + this.posX
      let y = Util.sin(this.angle + i * per) * this.radius + this.posY
